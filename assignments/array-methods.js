...
runners.filter((largeOut) => {
  if(largeOut.shirt_size === 'L'){
    runnersLargeSizeShirt.push(largeOut);
  }
})
...
// Problem 1
//what company each player are from

let playerCompany = [];

 runners.forEach((company) =>{
  playerCompany.push(`${company.first_name} ${company.last_name} is from ${company.company_name} Company`);
})

console.log(playerCompany);

// Problem 2
//what is player email and how much donation they made

const playerInfo = runners.map((info) => `${info.first_name} email is ${info.email} and he made ${info.donation} donation`);

console.log(playerInfo);


// Problem 3
// which runner made less donation then 150

const nameOrder = runners.filter((order) => order.donation < 150 )
